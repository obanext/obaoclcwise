...
function branchFilter(branchId) {
  return branchId ? `branchId_is_${branchId}` : "";
}
...
export default function OldSchoolSearchPage() {
  ...
  const [preselect, setPreselect] = useState(
    buildPreselectValue("catalog", DEFAULT_CATALOG_PERSPECTIVE_ID)
  );
  ...
  function submitSearch(event) {
    ...
    if (selected.type === "branch") {
      ...
      params.set("filter", branchFilter(selected.value));
    } else {
        ...
        <section className="old-school-search-panel">
          ...
          <form className="old-school-form" onSubmit={submitSearch}>
            ...
            >
              ...
              <optgroup label="Locaties Amstelland via Wise filter">
          ...
          <p className="old-school-debug-line">
            Actieve voorselectie: <strong>{selectedLabel}</strong>. Locatiekeuzes worden doorgestuurd
            als <code>filter=branchId_is_&lt;id&gt;</code>.
          </p>
        </section>
