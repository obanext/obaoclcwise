...
const AMSTELLAND_BRANCH_OPTIONS = [
  { label: "Amstelland - alle locaties", branchId: "" },
  { label: "Aalsmeer", branchId: "1000" },
  { label: "Amstelveen Stadsplein", branchId: "1001" },
  { label: "Amstelveen Westwijk", branchId: "1002" },
  { label: "Uithoorn", branchId: "1003" },
  { label: "Ouderkerk aan de Amstel", branchId: "1004" },
];
...
function parsePreselect(value) {
  const [type = "catalog", rawValue = DEFAULT_PERSPECTIVE_ID] = String(value || "").split(":");
...
export default function OldSchoolSearchPage() {
  ...
  const [preselect, setPreselect] = useState(buildPreselectValue("catalog", DEFAULT_PERSPECTIVE_ID));
  ...
  function submitSearch(event) {
    ...
    if (selected.type === "branch") {
      params.set("perspectiveId", DEFAULT_PERSPECTIVE_ID);
      if (selected.value) params.append("facetFilter", `branchId:${selected.value}`);
    } else {
      params.set("perspectiveId", selected.value || DEFAULT_PERSPECTIVE_ID);
    }
